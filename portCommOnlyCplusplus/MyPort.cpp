	...
	char data)
{
	...
	{
		sprintf(com_buff, "\\\\.\\%s", portname);
	}
	else
	{
		strcpy(com_buff, portname);
	}
	//映射设备名，避免设备名为ANSI字符（中文、韩文等字符）无法识别的情况
	int num = MultiByteToWideChar(0, 0, com_buff, -1, NULL, 0);//获取将com_buff映射成宽字符串后存放新字符串所需的缓存大小
	wchar_t *szPort = new wchar_t[num];//用于接收映射结果
	MultiByteToWideChar(0, 0, com_buff, -1, szPort, num);//进行映射
	
	//打开端口
	port_h = CreateFileA(szPort, //串口名
